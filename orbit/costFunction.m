...
function [cost,trajectory] = costFunction(x,data) 

% x = globalParam from NSGA
...
global counter
counter  = 0;
stop_fun = @(time,eoe) controlFunction(time, eoe, inputs, q_law_params);
...
for j = 1:solverDataSize 
    ...
    trajectory(j).Alpha = allData(j).p_alpha; 
    trajectory(j).Beta = allData(j).p_beta; 
    trajectory(j).Thrust = allData(j).p_thrust;
    trajectory(j).DeltaV = allData(j).p_total_dV;
    trajectory(j).Dt = allData(j).p_dt;
    trajectory(j).Eta_abs = allData(j).p_eta_absolute;
    trajectory(j).Eta_rel = allData(j).p_eta_relative;
    if allData(j).p_eclipse > 0 
        trajectory(j).InShadow = 1; 
    else 
        trajectory(j).InShadow = 0; 
    end 
    
    trajectory(j).Qvalue = compute_Q(trajectory(j).Eoe,targetState_eoe,inputs,q_law_params);
    trajectory(j).Qdvalue = compute_Qdot_min(trajectory(j).Eoe,targetState_eoe,trajectory(j).Eoe.L,inputs,q_law_params);
    
    trajectory(j).Coe = EOE_getClassicalElements(trajectory(j).Eoe);
    
    trajectory(j).pos = COE_getPosition(trajectory(j).Coe);
    trajectory(j).vel = COE_getVelocity(trajectory(j).Coe,inputs); 
    
    trajectory(j).inertialThrustDir = [cos(trajectory(j).Beta)*sin(trajectory(j).Alpha), cos(trajectory(j).Beta)*cos(trajectory(j).Alpha), sin(trajectory(j).Beta)];
    
    rstFrame = GetRSTFrame(trajectory(j).pos,trajectory(j).vel); 
    
    ITD = trajectory(j).inertialThrustDir;
    trajectory(j).inertialThrustDir(1) = dot(rstFrame(:,1),ITD);
    trajectory(j).inertialThrustDir(2) = dot(rstFrame(:,2),ITD);
    trajectory(j).inertialThrustDir(3) = dot(rstFrame(:,3),ITD); 
    
    trajectory(j).sunVector = getSolarPosition(allData(j).p_julianDate + times(j)/86400); 
    
    trajectory(j).Coe.o = wrapTo2Pi(trajectory(j).Coe.o); 
    trajectory(j).Eoe.L = wrapTo2Pi(trajectory(j).Eoe.L); 
