...
function [condition,converged,direction] = controlFunction_nsga(time, eoe, inputs, q_law_params) 
...
% referenceDeltaL = 0;% inputs.ref_delta_L; 
% 
...
%     step = 0; % hardcodeao CAMBIAAAAR   % ESTO ES EL COUNTER
...
Eclipse_enabled = inputs.eclipse_enabled; 
...
condition1 = Qvalue < 0.1; % (convergenceRadius*q_law_params.weightNormal);
if condition1
    thrust = 0; 
    %converged = 1; 
end
...
% if (min(min(etaAbs,etaRel),1) && eclipse > 0 && inputs.eclipse_thrust_level > 10^-6)
%     param.p_thrust = inputs.eclipse_thrust_level * maxThrust; 
% end 
...
condition = Qvalue - 0.1; % (convergenceRadius*q_law_params.weightNormal); 
converged = 1; 
direction = 0;
